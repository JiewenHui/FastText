...
def evaluateMRR(sess,dev_data):
    ...
    for batch_ids, batch_mask, batch_segment, batch_label,question_length,answer_length in batch_iter(dev_data,config.questions_num):
        batch_len = len(batch_ids)
        data_len+=batch_len
        feed_dict = feed_data(batch_ids, batch_mask, batch_segment, batch_label,question_length,answer_length, 1.0)
        loss, scores,labels = sess.run([model.loss, model.scores,model.labels], feed_dict=feed_dict)
        # results.append(scores)
        # if len(results) == 10:
        #     scores = np.stack(scores).reshape(-1,2)[:, 1]
        ...
        #             gt.append(i)
        #     gt = set(gt)
        #     all_metrics += metircs.metrics(
        #         gt=gt, pred=pred_docs, metrics_map=METRICS_MAP)
        #     print('----------------------')
        #     print('======================')
        #     asadas += 1
        #     tf.logging.info("  ".join(METRICS_MAP))
        #     tf.logging.info(all_metrics/asadas)
        #     print('----------------------')
            
        total_loss += loss * batch_len
        
    return total_loss/data_len,scores,labels
...
def train():
    ...
    for epoch in range(config.num_epochs):
        ...
        for batch_ids, batch_mask, batch_segment, batch_label,question_length,answer_length in batch_train:
            ...
            _, global_step, train_summaries, train_loss, train_accuracy ,scores ,labels = session.run([model.optim, model.global_step,
                                                                                    merged_summary, model.loss,
                                                                                    model.acc,model.scores,model.labels], feed_dict=feed_dict)


            if global_step % config.print_per_batch == 0:
                end = time.time()
                val_loss,val_accuracy=evaluate(session,dev_data)
                val_loss,scores,labels = evaluateMRR(session,dev_data)
                scores = np.stack(scores).reshape(-1,2)[:, 1]
                pred_docs = scores.argsort()[::-1]
                ...
                asadas += 1
                mrr_test = all_metrics[2]/asadas
                map_test = all_metrics[0]/asadas
                merged_acc=(train_accuracy+val_accuracy)/2
                if map_test > best_map:
                ...
                tf.logging.info("step: {},val map: {:.3f},val mrr@10: {:.3f},training speed: {:.3f}sec/batch {}".format(
                        global_step, map_test, mrr_test,(end - start) / config.print_per_batch,improved_str))
                start = time.time()
